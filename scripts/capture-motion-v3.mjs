...
const output = path.resolve(
  process.argv[2] ?? `.local/motion-v3-${Date.now()}`,
);
...
for (const mobile of [false, true]) {
  ...
  await page.screenshot({
    path: path.join(output, name + "-menu-afterglow.png"),
  });
  ...
  await page.waitForTimeout(1500);
  await page.screenshot({
    path: path.join(output, name + "-text-afterglow.png"),
  });
  ...
  await page.screenshot({
    path: path.join(output, name + "-cut-afterglow.png"),
  });
  ...
  await page.screenshot({
    path: path.join(output, name + "-small-copy-afterglow.png"),
  });
...
// Same headline, real reloads, unmodified random source. Record the draws even
// when a draw repeats; these are observations, not a uniqueness assertion.
await page.setViewportSize({ width: 1440, height: 900 });
notes.reloadPalettes = [];
for (let reload = 0; reload < 3; reload++) {
  await page.goto("http://127.0.0.1:3017/");
  await page.locator(".wonder-type").scrollIntoViewIfNeeded();
  await page.waitForTimeout(1700);
  notes.reloadPalettes.push(
    await page
      .locator(".wonder-type > .reveal-text")
      .evaluateAll((es) => es.map((el) => el.dataset.palette)),
  );
  await page.screenshot({
    path: path.join(output, "reload-" + (reload + 1) + ".png"),
  });
}
await browser.close();
await fs.writeFile(
