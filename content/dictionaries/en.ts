import type { Dictionary } from "./ja";
export const en: Dictionary = {
  common: {
    skip: "Skip to content",
    homeLink: "MYSTENA home",
    backTop: "Back to top",
    menu: "Menu",
    close: "Close",
    menuOpen: "Open menu",
    menuClose: "Close menu",
    siteNav: "Site navigation",
    primaryNav: "Main navigation",
    footerNav: "Footer navigation",
    language: "Language",
    japanese: "日本語",
    english: "English",
    unavailable: "English version unavailable",
    japaneseUnavailable: "Japanese version unavailable",
    englishNews: "English news",
    japaneseNews: "Japanese news",
    unsavedPrompt:
      "Switching languages will discard your unsent changes. Continue?",
    tagline: "Entertainment × Technology",
    brandNote: "Be curious. Find your next.",
    navMessage: "Spark curiosity.\nOpen new possibilities.",
    copyright: "© MYSTENA",
    home: "Home",
    required: "Required",
    optional: "Optional",
    motion: {
      pause: "Pause animations",
      play: "Play animations (respects reduced-motion settings)",
      pausedTitle: "Animations paused. Your system preference takes priority.",
      label: "Motion",
      on: "ON",
      off: "OFF",
    },
  },
  review: {
    label: "Content review",
    notice:
      "Business descriptions are proposed copy. Service status and legal company information await confirmation.",
    candidate: "Proposed copy",
    pending: "To be confirmed",
    stage: "Copy assumes development is in progress",
    collaboration: "Proposed collaboration topics",
    companyIncomplete: "Company information pending confirmation",
    assetsPending: "Approved material pending",
  },
  pages: {
    home: {
      title: "Create engaging experiences. Bring their appeal to life.",
      menu: "Home",
      short: "Top",
      description:
        "Explore MYSTENA’s proposed business, approach, current initiatives and company information. This corporate site is a content review edition.",
    },
    about: {
      title: "About us",
      menu: "About us",
      short: "About",
      description:
        "Turning engaging ideas into intuitive services. Discover the principles that guide MYSTENA.",
    },
    business: {
      title: "Our business",
      menu: "Our business",
      short: "Business",
      description:
        "Explore MYSTENA’s four proposed business areas: entertainment commerce, service and system development, video and content production, and marketing and collaboration.",
    },
    company: {
      title: "Company information",
      menu: "Company information",
      short: "Company",
      description:
        "Information about the organization behind MYSTENA. Legal company details await confirmation.",
    },
    news: {
      title: "News",
      menu: "News",
      short: "News",
      description: "Confirmed announcements from MYSTENA.",
    },
    contact: {
      title: "Contact",
      menu: "Contact",
      short: "Contact",
      description:
        "Contact information for MYSTENA. Our contact channel is being prepared.",
    },
    privacy: {
      title: "Privacy",
      menu: "Privacy policy",
      short: "Privacy",
      description:
        "Information about how MYSTENA handles personal information. Our formal policy is being prepared.",
    },
  },
  home: {
    heroLines: ["Create engaging experiences.", "Bring their appeal to life."],
    description:
      "MYSTENA brings entertainment and technology together through service development, video production, and marketing. From engaging online shopping experiences to business systems and projects with brands and creators, we connect planning, development, creative work, and promotion to create new experiences.",
    businessButton: "Explore our business",
    companyButton: "View company information",
    scroll: "Scroll to explore",
    businessTitle: "Making product discovery\nmore engaging.",
    businessDescription:
      "From creating products and services to communicating their appeal and encouraging people to use them, we bring four areas together to explore new experiences.",
    businessStructure:
      "Four areas connecting planning, development, creative work, and promotion.",
    workingTitle: "What we are working on",
    workingDescription:
      "We are developing an online service designed to help people discover products in new ways. We will share further details and availability when they are ready.",
    workingNote:
      "This proposed copy assumes development is in progress. The actual service status and scope of disclosure await confirmation.",
    philosophyLabel: "What matters to us",
    philosophyLines: ["A little wonder", "changes everything."],
    philosophyDescription:
      "Unexpected discoveries can make everyday life a little richer. We use technology and ideas to create opportunities for those moments.",
    aboutButton: "Explore our approach",
    companyDescription: "Learn about the organization behind MYSTENA.",
  },
  about: {
    heading: "Turning engaging ideas\ninto intuitive services.",
    paragraphs: [
      "MYSTENA aims to create new connections between people, products, and content. We develop ideas that spark interest into services people can use. To do this, we approach planning, development, and creative expression as parts of one experience.",
    ],
    valuesTitle: "What matters to us",
    values: [
      {
        title: "Stay curious.",
        description:
          "We look for overlooked qualities and new opportunities for discovery.",
      },
      {
        title: "Make things clear.",
        description:
          "We aim for language people understand and interfaces they can use with confidence.",
      },
      {
        title: "Build trust over time.",
        description:
          "We explain our services and their terms clearly, and use feedback to improve.",
      },
    ],
  },
  business: {
    factsTitle: "Defining the online service concept",
    audienceLabel: "Who it is for",
    audience:
      "Businesses offering products or content, and people who enjoy discovering new products",
    deliverableLabel: "What we plan to offer",
    deliverable:
      "An online service and the presentation and user experience within it",
    stageLabel: "Service status",
    stage:
      "To be confirmed (the proposed copy assumes development is in progress)",
    revenueLabel: "Revenue model",
    revenue: "The information to be disclosed is being confirmed.",
    areas: {
      platform: {
        role: "Business area 01",
        title: "Entertainment\ncommerce",
        description:
          "We plan, develop, and operate online sales services that make shopping more engaging. We explore experiences that make both discovering products and shopping enjoyable.",
        consultation: "Plan a more engaging online sales service",
        visualAlt:
          "A 2D illustration of a woman choosing products in an online shop",
        detailVisualAlt:
          "A 2D illustration of a woman enjoying online shopping with a laptop product grid, headphones and a delivery box",
      },
      systems: {
        role: "Business area 02",
        title: "Service and system\ndevelopment",
        description:
          "We develop services for businesses, sales and operations management systems, and web services. We turn business needs into systems that are easy to use.",
        consultation: "Develop a sales service or an operations system",
        visualAlt:
          "A 2D illustration of two developers writing code and building a web interface",
        detailVisualAlt:
          "A 2D illustration of developers working with a code editor, a live UI preview, a mobile interface and wireframe sketches",
      },
      creative: {
        role: "Business area 03",
        title: "Video and content\nproduction",
        description:
          "We produce in-service visual sequences, product introduction videos, advertising materials, and digital content. We develop creative approaches that communicate what makes a product or service appealing.",
        consultation: "Create a video introducing a product or service",
        visualAlt:
          "An AI-generated concept image of a product camera setup and video editing screen",
        detailVisualAlt:
          "An AI-generated concept image of a camera filming a lit blue bottle beside a video preview and editing timeline",
      },
      marketing: {
        role: "Business area 04",
        title: "Marketing and\ncollaboration",
        description:
          "We plan sales promotions, social media initiatives, influencer partnerships, and joint projects with brands and creators. These can include product curation, limited-edition product development, video production, and sales campaigns.",
        consultation: "Co-create a product with a brand or creator",
        visualAlt:
          "A 2D illustration of a creator collaboration promoting a product",
        detailVisualAlt:
          "A 2D illustration of a creator presenting a product to a smartphone while a collaborator reviews packaging and social media images",
      },
    },
    consultationLabel: "Example inquiry",
  },
  diagram: {
    title: "Entertainment commerce concept",
    caption:
      "A proposed overview of audiences and what the service could offer. The actual business scope awaits confirmation.",
    nodes: [
      {
        title: "Businesses offering products",
        description: "The appeal of products and content",
      },
      {
        title: "MYSTENA’s service",
        description: "Designing discovery and the user experience",
      },
      {
        title: "People looking for something new",
        description: "The enjoyment of discovery",
      },
    ],
  },
  collaboration: {
    title: "Exploring new ways to bring\nproducts and content to people.",
    intro:
      "We bring together strengths in products, content, and technology to explore new experiences.",
    description:
      "Co-create a product, develop a sales service, or produce an introduction video. These proposed inquiry topics connect planning, development, creative work, and promotion.",
    themes: [
      "Co-developing or curating products and limited editions",
      "Developing sales services and business systems",
      "Producing product videos and advertising materials",
      "Social media initiatives and influencer partnerships",
    ],
    note: "The collaboration topics we can accept are being confirmed. Our contact channel is being prepared.",
    button: "Collaboration inquiries",
  },
  company: {
    intro: "Key information about the organization behind MYSTENA.",
    notice:
      "Legal company information is being confirmed. We will publish confirmed details when they are ready. MYSTENA is a brand name, not a substitute for a legal company name.",
    fields: {
      brand: "Brand",
      legalName: "Legal company name",
      englishName: "English company name",
      representative: "Representative",
      address: "Registered address",
      establishedAt: "Established",
      capitalYen: "Capital",
      businessActivities: "Business activities",
      corporateNumber: "Corporate number",
      email: "Contact",
      telephone: "Telephone",
    },
    photoTitle: "A picture of creating together.",
    photoNote:
      "An illustration of work connecting planning, development and creative expression. The people and office are imaginary.",
  },
  contact: {
    noticeTitle: "Our contact channel is being prepared.",
    notice:
      "The form is not currently accepting input or submissions. We will provide contact details when they are ready.",
    fieldset: "Inquiry details (not yet accepting submissions)",
    purpose: "Inquiry type",
    select: "Please select",
    purposes: ["Business and services", "Collaboration", "Media", "Other"],
    company: "Company or organization",
    name: "Name",
    email: "Email address",
    message: "Message",
    privacyBefore:
      "Information about how we handle personal information will be published in our ",
    privacyAfter: ".",
    disabledSubmit: "Not yet accepting inquiries",
    submit: "Send inquiry",
    requiredError: "Please complete the required fields.",
    emailError: "Please enter a valid email address.",
    success:
      "Your inquiry has been received. We will review your message and get back to you.",
    failure: "We could not send your inquiry. Please try again later.",
    unconfigured:
      "Our contact channel is being prepared. Your inquiry has not been sent.",
  },
  contactBand: {
    title: ["New possibilities begin", "with a conversation."],
    description: "Our contact channel is being prepared.",
  },
  news: {
    emptyTitle: "There are no announcements at this time.",
    emptyDescription:
      "Confirmed announcements will appear here when they are ready.",
    all: "View all news",
    back: "Back to news",
    home: "Back to home",
  },
  privacy: {
    status: "Being prepared",
    notice:
      "Our formal policy on handling personal information will be published here once it has been confirmed.",
    explanation:
      "The contact form is not currently accepting personal information or submissions. When the channel opens, we will explain the purposes of use and provide contact details.",
    contact: "About contacting us",
  },
  notFound: {
    title: "Page not found.",
    description:
      "The address may have changed, or the page may not have been published yet. Please return to the home page.",
    back: "Back to home",
  },
  assets: {
    expand: "Enlarge image (opens in a new tab)",
    aboutCaption:
      "A desk where ideas take shape. An illustration connecting planning, development and production.",
    posterLabel: "Service introduction concept",
    posterTitle: "Discover a collection. Open a box.",
    posterNote:
      "A video cover concept introducing product discovery and the unboxing experience. Only this still image is available; the video is being prepared.",
    afterglowAlt: "An abstract brand visual with a loop of blue light",
    daylightAlt: "An abstract transparent form in soft overlapping light",
    slots: [
      {
        title: "A new way to discover collectibles.",
        description:
          "Trading cards, brand fashion and precision devices. A service concept exploring the enjoyment of product discovery through mystery boxes.",
        label: "Service concept 01",
      },
      {
        title: "Understand. Choose. Unbox.",
        description:
          "Review product information and conditions, choose a box, open it, and learn about the product you will receive. A concept for an enjoyable experience built on clarity.",
        label: "Experience concept 02",
      },
    ],
    note: "These illustrations explain the service concept. Actual products, screens and service details will be announced once confirmed.",
  },
};
