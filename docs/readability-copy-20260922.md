...
| 18 | `diagram.*` | **REFINE**: `つながり` → `事業者と利用者の関係`; nodes state who introduces products, designs presentation/usability and chooses products. | The shorter title avoids an isolated final character at 390px; MYSTENA's role remains explicit in the caption and center node. HTML explains the relationship without image enlargement. No new transaction, payment or shipment relationship. |
