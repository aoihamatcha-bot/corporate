...
const output = path.resolve("docs/evidence/gradient-followup-20260922/final");
const builtSourceHead = process.env.CAPTURE_SOURCE_SHA;
const expectedBuildId = "Wdc_zTNX4t7zHF0FLGcpR";
// Prepare this script without executing it. The implementation owner supplies
// the final committed source SHA and the explicit start signal before capture.
if (!builtSourceHead || !/^[a-f0-9]{40}$/.test(builtSourceHead)) {
  throw new Error("CAPTURE_SOURCE_SHA must be the owner-confirmed final source commit");
}
const actualHead = execFileSync("git", ["rev-parse", "HEAD"], { encoding: "utf8" }).trim();
if (actualHead !== builtSourceHead) throw new Error("Current HEAD differs from CAPTURE_SOURCE_SHA");
const actualBuildId = (await fs.readFile(".next/BUILD_ID", "utf8")).trim();
if (actualBuildId !== expectedBuildId) throw new Error("Unexpected production build ID");
const previousCaptures = (await fs.readdir(output)).filter((name) => /\.(png|webm)$|^(partial-)?record\.json$/.test(name));
if (previousCaptures.length) throw new Error("Refusing to overwrite existing final captures");
...
const report = {
  ...
  builtSourceHead,
  builtSourceBinding: "Final production build supplied by implementation owner and capture explicitly started against CAPTURE_SOURCE_SHA. Current HEAD and expected build ID were verified before browser launch. Immutable runtime files, 133-file source scope, and served JS/CSS are hashed below.",
