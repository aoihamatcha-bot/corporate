...
青・水色・白を中心にした、日英対応のMYSTENAコーポレートサイト。Next.js App Router / TypeScript / Reactを使用します。
...
- 今回の原稿・構成と25項目の対応: [可読性改修の原稿記録](docs/readability-copy-20260922.md)
- 今回の検証・画面・残る公開Gate: [可読性改修の検証](docs/verification-readability-20260922.md)
- その後の文字・背景グラデーション調整: [2026-09-22追加改修](docs/gradient-followup-20260922.md)
...
本文・補足・ラベル・ナビゲーションは初めから読める原文にグラデーション色を重ね、保持してから通常色へ戻します。本文の位置は動かさず、色演出は初回のみ、メニューは開くたびに再生します。見出しは指定配色の登場演出を維持。背景は各セクションの指定配色に固定し、淡い面と柔らかい帯で青・白の構成を支えます。OSの動き抑制設定に対応し、サイト内のON/OFF操作は設けていません。

TOPの手書きオープニングは更新でも再生し、完成後1秒保持して見出しへ着地します。HEROは静的な文字グラデーションを適用し、着地後に静止します。下部の事業の帯を経て、01事業紹介からスクロールに合わせて演出を開始します。カーソル追従線はありません。最新画面・動作動画はproduction server起動後に `node scripts/capture-hero-v4.mjs` で取得できます。確認用に名前を付けた動画をGitで管理し、重複する一時録画 `.recordings/` はローカルに保持します。
...
ビルド時はGoogle Fontsへの接続が必要ですが、閲覧者のブラウザからGoogle Fontsへの通信は発生しません。フォントが読み込めない場合も本文は端末の代替書体で表示されます。見出しのグラデーション帯はフォント読み込み完了後に行位置を計測します。本文は帯を作らず色だけを変えます。ロゴの字形とサムネイルのホバー動作は維持しています。
...
既存の採用素材を保持しています。Topは4事業の短い要約とHTMLの関係図を表示し、詳細な説明・サービス構想図・動画表紙の静止画はBusinessにまとめました。HTMLの関係図はPCで横、スマホで縦に読めます。細かい図版は補助資料として新しいタブで原寸を確認できます。
...
A01・A02・A06・A07は、提供素材の採用後、日英の構図を揃えたv4へ更新しています。確認用の下端注記を整理し、英語版の未置換箇所も更新しました。[現在の素材・ハッシュ](docs/asset-revisions-v4.json)。以前の差し替え記録は [v3対応表](docs/concept-replacements-v3.json) / [v3検証](docs/concepts-and-metrics-v3.md) に保持しています。
