...
本文・補足・ラベル・ナビゲーションは初めから読める静止表示です。見出しの登場演出は指定配色で一度だけ再生し、文字色の余韻は0.4〜0.6秒。画像のホバー配色と背景のパステルは維持しています。OSの動き抑制設定に対応し、サイト内のON/OFF操作は設けていません。

TOPの手書きオープニングは更新でも再生し、完成後1秒保持して見出しへ着地します。HEROは着地後に静止し、下部の事業の帯を経て、01事業紹介からスクロールに合わせて演出を開始します。カーソル追従はありません。最新画面・動作動画はproduction server起動後に `node scripts/capture-hero-v4.mjs` で取得できます。確認用に名前を付けた動画をGitで管理し、重複する一時録画 `.recordings/` はローカルに保持します。
